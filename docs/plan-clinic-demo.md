...
**เสร็จเมื่อ** — ✅ login 3 role แล้วเห็นข้อมูลไม่เท่ากันจริง (ยืนยันแล้ว: ADMIN/STAFF ได้ 12 ฟิลด์ · VIEWER ได้ 4 ฟิลด์)

#### สิ่งที่ทำจริงใน Phase 1

| งาน                    | ทำจริงเป็นอะไร                                                                                                      |
| ---------------------- | -------------------------------------------------------------------------------------------------------------------- |
| Schema เต็ม            | 11 ตาราง + 5 enum · migration `full_schema` ผ่านแล้ว                                                                |
| เข้าสู่ระบบ            | JWT access 15 นาที + refresh 7 วัน · **หมุน refresh ทุกครั้งที่ใช้** ใบเดิมใช้ซ้ำไม่ได้                             |
| ตัดสิทธิ์              | ออกจากระบบเฉพาะอุปกรณ์นี้ / ทุกอุปกรณ์ — ทำได้เพราะเก็บ refresh token ลงตาราง (เก็บเป็น hash)                       |
| ปิดทุก endpoint ไว้ก่อน | `JwtAuthGuard` + `RolesGuard` เป็น guard ระดับแอป เปิดเฉพาะที่ติด `@Public()` อย่างจงใจ                             |
| PDPA serializer        | `CustomerResponseDto.from(customer, role)` — VIEWER ไม่ได้รับเบอร์โทร บันทึก ประวัติการมา และสถานะความยินยอม        |
| CRUD                   | ลูกค้า / ช่าง / บริการ · แบ่งหน้ามาตรฐาน · soft delete ทุกตัว                                                       |
| ความยินยอม             | endpoint แยกต่างหาก และทับ `consentAt` ใหม่ทุกครั้งที่ค่าเปลี่ยน                                                     |
| Seed                   | ผู้ใช้ 3 สิทธิ์ · ช่าง 3 · บริการ 8 · ลูกค้า 20 (ผูก LINE 13 · ยินยอมการตลาด 11 · หายเกิน 90 วัน 12)               |
| เทสต์                  | 45 เคส ผ่านทั้งหมด                                                                                                  |

**บัญชีสำหรับเดโม** (รหัสผ่าน `demo1234` ทุกบัญชี — ของจริงตอนส่งมอบต้องเปลี่ยน)

| อีเมล                   | สิทธิ์ | เห็นอะไร                                     |
| ----------------------- | ------ | -------------------------------------------- |
| `owner@thnpclinic.com`  | ADMIN  | ทุกอย่าง รวมถึงจัดการช่าง/บริการ/ปิดบัญชีลูกค้า |
| `staff@thnpclinic.com`  | STAFF  | ข้อมูลลูกค้าครบ แต่แก้ช่าง/บริการไม่ได้        |
| `demo@thnpclinic.com`   | VIEWER | ชื่อลูกค้าเท่านั้น ไม่เห็นเบอร์และประวัติ       |

#### สองเรื่องที่ตัดสินใจระหว่างทาง

**ไม่ใช้ Passport** — สำหรับ JWT ล้วน ๆ การเขียน guard เองสั้นกว่าและอ่านออกในไฟล์เดียว ปลายทางคือลูกค้ารับโค้ดไปดูแลต่อ การมี strategy/module ของ Passport มาคั่นทำให้ต้องไล่อ่านหลายชั้นโดยไม่ได้อะไรเพิ่ม

**ซ่อนข้อมูลด้วยฟังก์ชันธรรมดา ไม่ใช้ decorator** — `CustomerResponseDto.from()` เขียนกฎว่าใครเห็นอะไรไว้ตรง ๆ ให้อ่านจบในบรรทัดเดียว ต่างจากการใช้ `@Exclude`/`groups` ที่ต้องรู้กลไกของ class-transformer ก่อนถึงจะรู้ว่าข้อมูลถูกกันจริงหรือเปล่า — และเทสต์ยืนยันว่าฟิลด์ที่ปิด **ไม่มี key อยู่ใน response เลย** ไม่ใช่แค่มีค่าเป็น undefined
...
**เสร็จเมื่อ** — ✅ สร้าง ย้าย ยกเลิก ปิดงาน ได้ครบ และ **ยิงจองพร้อมกัน 8 ครั้งแย่งช่องเดียวกัน จองติดแค่ 1** (อีก 7 ได้ 409 SLOT_TAKEN)

#### สิ่งที่ทำจริงใน Phase 2

| งาน                | ทำจริงเป็นอะไร                                                                                                  |
| ------------------ | ---------------------------------------------------------------------------------------------------------------- |
| State machine      | `appointment-state-machine.ts` — ตารางเดียวคุมทุกการเปลี่ยนสถานะ ทดสอบครบทั้ง 36 คู่ที่เป็นไปได้                |
| กันจองซ้อน 2 ชั้น  | advisory lock ต่อช่างใน transaction (ชั้นแอป) + exclusion constraint (ชั้นฐานข้อมูล)                            |
| ย้ายนัด            | ยกเลิกใบเดิม + ออกใบใหม่ใน transaction เดียว ประวัติการย้ายยังตรวจสอบได้                                        |
| ปิดงาน             | เปลี่ยนสถานะ + อัปเดต `lastVisitAt` + ตัดครั้งคอร์ส ทั้งหมดอยู่ใน transaction เดียว                             |
| บอร์ดคิว           | `GET /appointments/day-board` — คิวแยกตามช่าง + นับแยกสถานะ + รายได้ที่คาดว่าจะได้เทียบกับที่ได้จริง            |
| ช่องเวลาว่าง       | `GET /appointments/availability` — คืนเฉพาะช่องที่ยาวพอสำหรับบริการที่เลือก                                     |
| เวลาไทย            | `bangkok-time.ts` คำนวณ +07:00 ตรง ๆ ไม่พึ่ง TZ ของ process                                                     |
| เทสต์              | 119 เคส (เพิ่มจาก Phase 1 อีก 74)                                                                               |

#### กันจองซ้อนทำไมต้องสองชั้น

**ชั้นแอป — advisory lock** การถามว่า "ช่วงเวลานี้ว่างไหม" คืออ่านแถวที่ยังไม่มีอยู่ `SELECT ... FOR UPDATE` จึงล็อกอะไรไม่ได้เลย สองคำขอที่เข้ามาพร้อมกันเห็นว่าว่างทั้งคู่แล้วเขียนทับกัน — แก้ด้วยการล็อกที่ตัว "ช่างคนนี้" แทนด้วย `pg_advisory_xact_lock` คำขอที่สองต้องรอจนคำขอแรกเขียนเสร็จ

**ชั้นฐานข้อมูล — exclusion constraint** ชั้นแรกกันได้เฉพาะทางที่ผ่านโค้ดเรา ชั้นนี้กันครอบทุกทาง รวมสคริปต์นำเข้าข้อมูลและการแก้มือผ่าน SQL:

```sql
EXCLUDE USING gist ("providerId" WITH =, tsrange("startsAt","endsAt",'[)') WITH &&)
  WHERE (status IN ('BOOKED','CONFIRMED'))
```

ทดสอบแล้วทั้ง 3 เคส: แทรกนัดซ้อนตรงผ่าน SQL → ถูกปฏิเสธ · นัดที่ยกเลิกแล้วทับเวลาได้ · นัดต่อกันพอดี 11:00 ไม่นับว่าชน (เพราะใช้ขอบเขต `[)`)

#### สองจุดที่เจอตอนลงมือ

**`tstzrange` ใช้ไม่ได้ ต้องเป็น `tsrange`** — Prisma เก็บ `DateTime` เป็น `timestamp without time zone` การแปลงเป็น `timestamptz` ขึ้นกับค่า TimeZone ของ session ทำให้ Postgres ถือว่าไม่ immutable แล้วสร้าง index ไม่ได้ (`ERROR: functions in index expression must be marked IMMUTABLE`)

**ย้ายนัดต้องออกใบใหม่ ไม่ใช่แก้เวลาในใบเดิม** — เพราะงานเตือนนัดที่ตั้งไว้กับใบเดิมต้องถูกยกเลิกไปพร้อมกัน (Phase 4) ถ้าแก้ทับ งานเก่าจะยังค้างอยู่แล้วยิงเตือนผิดเวลา ซึ่งเป็นความพังแบบเงียบที่กว่าจะรู้ก็คือลูกค้าได้รับข้อความผิดไปแล้ว
